...
# some function for training, evaluation and testing        

# This function will test the best model over the specified testset and delivers 
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for images, labels, img_names, meta_info,patch_densenet_gradcam_importance_weights in tqdm(test_loader, desc="Testing", leave=False):
            ...
            all_patch_densenet_gradcam_importance_weights.extend(patch_densenet_gradcam_importance_weights.cpu().numpy())  # true labels
    ...
    # Return the results
    return accuracy, patches_names, true_labels, predictions, top_probabilities, wsi_names, patches_indices, patches_coordinates, all_patch_densenet_gradcam_importance_weights
...
def valid(args, model, writer, val_loader, global_step):
    ...
    epoch_iterator = tqdm(val_loader,
                          ...
                          dynamic_ncols=True,
                          disable=args.local_rank not in [-1, 0]) #to show a progress bar while iterating over the dataset during validation. desc =description text that appears at the beginning of the progress bar
                                                                   #"{l_bar}{r_bar}": This tells tqdm to only show the left and right parts of the progress bar
                                                                   #dynamic_ncols = adjusts the width of the progress bar
    loss_fct = torch.nn.CrossEntropyLoss() #This loss function is typically used for multi-class classification problems. It combines nn.LogSoftmax() and nn.NLLLoss() (Negative Log Likelihood Loss) into one function
    ...
    for step, batch in enumerate(epoch_iterator): # STEP IS INDEX OF BATCH, 
        ...
        with torch.no_grad():
            ...
            eval_loss = loss_fct(logits, labels) #The loss function (loss_fct) is applied to the logits and the true labels (y) to compute the loss. computes the cross-entropy loss between the predicted logits and the true class labels.
            eval_losses.update(eval_loss.item()) #This function is called to update a running metric for tracking the evaluation loss over all batches. It typically keeps track of the average loss over the evaluation phase.
                                                #eval_loss is a tensor, and calling .item() extracts the raw scalar value (a Python float) from the tensor. This is necessary because the update method requires a scalar value, not a tensor.
            preds = torch.argmax(logits, dim=-1) #computes the predicted class labels (preds) from the logits by taking the index of the maximum value along the last dimension (dim=-1).
                                                #logits = [[1.2, 0.3, 2.5], [0.5, 2.1, 1.8]], then preds = [2, 1]
        if len(all_preds) == 0:
            all_preds.append(preds.detach().cpu().numpy()) #Convert the tensor preds into a NumPy array and append it to all_preds.
                                                           #Removes the tensor from the computation graph. This means gradients will no longer be tracked for this tensor, which is unnecessary since we are now handling predictions for evaluation, not training.
            all_label.append(labels.detach().cpu().numpy())
        else:
            all_preds[0] = np.append(
                all_preds[0], preds.detach().cpu().numpy(), axis=0 
            ) #If all_preds is not empty (i.e., we have already processed at least one batch), we append the new batch of predictions and labels to the existing ones.
              #np.append(all_preds[0], preds.detach().cpu().numpy(), axis=0): Purpose: Append the new batch of predictions (preds) to the existing predictions (all_preds[0]), along the first axis (axis=0).
...
def train_test(args, model, train_loader, val_loader, test_loader, tune_loader, fold):
    ...
    test_accuracy, patches_names, true_labels, predictions, top_patches_probabilities, wsi_names, patches_indices, patches_coordinates, all_patch_densenet_gradcam_importance_weights = test(model, test_loader)
    _, tune_patches_names, tune_true_labels, tune_predictions, tune_top_patches_probabilities, tune_wsi_names, tune_patches_indices, tune_patches_coordinates, tune_all_patch_densenet_gradcam_importance_weights = test(model, tune_loader)
    ...
    "Densenet_Gradcam_Patch_Importance": tune_all_patch_densenet_gradcam_importance_weights,
    ...
    # 3) GradCAM-weighted voting
    for t in ths3:
        y_true, y_pred = [], []
        for wsi, grp in patch_df.groupby("WSI_Name"):
            true_lbl = int(grp["True_patch_Label"].any())
            w        = grp["Densenet_Gradcam_Patch_Importance"].to_numpy()
            fusion   = np.where(grp["Prediction"] == 1, 1, -1) * w
            pred_lbl = int(fusion.mean() > t)
            y_true.append(true_lbl)
            y_pred.append(pred_lbl)
        acc = float((np.array(y_true) == np.array(y_pred)).mean())
        old_acc, old_t = best["gradcam"]
        if acc > old_acc or (acc == old_acc and t > old_t):
            best["gradcam"] = (acc, t)
    ...
    fold_thresholds = {
        ...
        "softmax":  best["softmax"][1],
        "gradcam":  best["gradcam"][1]
    }
    ...
    return train_epoch_losses, train_losses, val_losses, val_accuracies, best_acc, test_accuracy, patches_names, true_labels, predictions, top_patches_probabilities, wsi_names, patches_indices, patches_coordinates, all_patch_densenet_gradcam_importance_weights, fold_thresholds
